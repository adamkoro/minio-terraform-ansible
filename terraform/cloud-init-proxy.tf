...
EOT
  ...
  meta_data = <<EOT
instance_id: ${sha1("${var.proxmox_proxy_vm_name}-${count.index + 1}")}
EOT
...
fqdn: ${var.proxmox_proxy_vm_name}-${count.index + 1}.${var.cloud_init_domain}
write_files:
...
network:
  ...
  ethernets:
    enp6s18:
      dhcp4: no
      addresses:
        - ${var.cloud_init_ip_pool0}${count.index + var.cloud_init_proxy_ip_increase}/${var.cloud_init_netmask}
      nameservers:
    ...
    enp6s19:
      dhcp4: no
      addresses:
        - ${var.cloud_init_ip_pool1}${count.index + var.cloud_init_proxy_ip_increase}/${var.cloud_init_netmask}
      nameservers:
