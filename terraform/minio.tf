resource "libvirt_domain" "minio" {
  ...
  disk {
    volume_id = element(libvirt_volume.data_disk.*.id, count.index)
  }
  network_interface {
    bridge = "br0"
  }
...
fqdn: ${var.minio_domain_name}-${count.index + 1}.${var.cloud_init_search_domain}
...
config:
    - type: physical
      ...
      subnets:
      - type: static
        address: '${var.node_ip_range}${count.index + 6}'
